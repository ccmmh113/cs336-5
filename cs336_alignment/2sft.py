...
def run_sft_experiment(args):
    ...
    总训练数据量 = max_steps ×batch_size Logical Batch Size
    '''
    ...
    total_train_steps=args.max_steps+100
    ...
    # ----------------------------------------------------------
    print(f"数据加载 {args.train_data_path}...")
    raw_train_data = []
    ...
    if args.dataset_size and args.dataset_size < len(raw_train_data):
        ...
        print(f"开始采样: {args.dataset_size}")
    ...
    print(f"数据处理完成，一共 {len(tokenized_train_data['input_ids'])}条")
    ...
    save_name = f"sft_steps{args.max_steps}_subset{args.dataset_size}"
...
if __name__ == "__main__":
    ...
    parser.add_argument("--train_data_path", type=str, default="data/gsm8k/sft.jsonl")
